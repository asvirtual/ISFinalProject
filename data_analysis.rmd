...
\begin{itemize}
  ...
  \item Can we quantify the impact of individual factors such as effort (study time) and external support (paid classes) on the final Mathematics grade (G3)?
\end{itemize}
\vspace{-10px}
...
```{r echo=FALSE, fig.height=5, fig.width=10, out.width="65%", fig.align="center"}
library(tidyr)
...
\vspace{-10px}
```{r echo=FALSE, fig.width=8, fig.height=5, out.width="70%", fig.align="center"}
hist(data$absences, main="Distribution of Absences", xlab="Number of Absences")
shapiro_test <- shapiro.test(data$absences)
```

As shown in the histogram, the variable follows a right-skewed distribution (strongly asymmetric). Then, the Shapiro-Wilk's test returns a p-value = `r shapiro_test$p.value` lower than $\alpha = 0.05$ that confirms non-normality. 

Given this information, it is better to compute a Spearman test, which does not require a normal distribution of the data.

```{r include=FALSE, fig.width=8, fig.height=5, out.width="70%", fig.align="center"}
...
The Spearman test resulted in a high p-value of `r round(cor_spearman_absences$p.value, 3)` (calculated via asymptotic approximation due to data ties), leading to the non-rejection of the null hypothesis ($H_0$). This indicates that no statistically significant correlation exists between `absences` and final grades in this dataset, refuting the hypothesis that increased absences lower student performance.
...
We observed 38 "dropout" entries (`G3`=0). We re-tested the Improved Model on filtered data, called **Filtered Model**, to ensure results weren't driven solely by these outliers.
...
1.  **Academic Effort Model:** Predictors `studytime`, `absences`, `failures`. ($R^2 \approx `r round(summary(model_a)$r.squared * 100, 1)` \%$)
2.  **Background Model:** Predictors `Medu`, `famrel`, `internet`. ($R^2 \approx `r round(summary(model_b)$r.squared * 100, 1)` \%$)
3.  **Optimized Model (Stepwise):** Selected via AIC. ($R^2 \approx `r round(summary(best_model)$r.squared * 100, 1)` \%$)
...
1.  **Validity**: The refined model's $R^2$ increased to **`r round(summary(refined_model)$r.squared * 100, 1)`\%** (from ~`r round(summary(best_model)$r.squared * 100, 1)`\%), and the Q-Q plot (top right) confirms normality is satisfied.
2.  **The "Absences" Flip**: In the refined model, the coefficient for absences flips to **negative ($\approx `r round(coef(refined_model)["absences"], 3)`$)** (in the original model it was $`r round(coef(best_model)["absences"], 3)`$), proving that for active students, missing class does hurt performance.
3.  **Selection Bias in School Support**: The negative effect of school support ($\approx `r round(coef(refined_model)["schoolsupyes"], 3)`$) suggests it acts as a proxy for at-risk students.
...
While the "Effort" model hypothesized that weekly study hours would be a primary driver of grades, our regression analysis showed that `studytime` was not statistically significant when controlling for past performance. In contrast, every past failure reduced the expected final grade by approximately 1 point.
...
\vspace{-5px}
*   **Romantic Relationships**: We observed a significant penalty for students in relationships (>1 mean grade point drop). This supports the "resource allocation" theory, where the time and emotional energy demanded by relationships compete directly with academic focus.
